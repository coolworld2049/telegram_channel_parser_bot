...
def initial_selection(api_id, app_hash):
    ...
    session_maker(createbot, api_id, app_hash, os.getenv("PHONE_NUMBER") or None)


def fill_api():
    clear()
    while True:
        api_id = int(os.getenv("API_ID")) or input("Insert app_id: ")
        if str(api_id).isdigit():
            break
        print("Invaild app_id!\n")

    while True:
        app_hash = os.getenv("API_HASH") or input("Insert api_hash: ")
        if app_hash:
            break
